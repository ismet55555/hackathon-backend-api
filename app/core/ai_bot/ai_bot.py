...
user: {description}  make sure to base it on the { businessInfo }."""
...
user:
I want to do an instragram caption about { intent } make sure to base it on the { businessInfo }."""
...
user: Create an image about { intent } for the { modelName } image generator make sure to base it on the { businessInfo }.
"""
...
user: { elevatedPrompt }"""
    ...
    async def understand_intent(self):
        ...
        result = await client.chat.completions.create(
            model="gpt-3.5-turbo", messages=[{"role": "user", "content": self.intentPrompt}]
        )
